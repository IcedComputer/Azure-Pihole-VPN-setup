...
DISALLOW_OVPN12=$(cat /etc/pivpn/DISALLOW_OVPN12)
...
helpFunc() {
	...
	#Only display if user has not installed 2.4 and is iOS OVPN12 compatable
	if [[ $DISALLOW_OVPN12 == false ]]; then
...
if [ "$iOS" = "1" ]; then
	...
	printf "========================================================\n"
	openssl pkcs12 -passin env:"$PASSWD_UNESCAPED" -export -in issued/${NAME}${CRT} -inkey private/${NAME}${KEY} -certfile ${CA} -name ${NAME} -out /home/$INSTALL_USER/ovpns/$NAME.ovpn12
